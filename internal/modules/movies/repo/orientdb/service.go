...
func (s *service) CheckDB() error {
	...
	if err != nil {
		zap.S().Error("incorrect request", err)
		return err
	}
...
func (s *service) CreateMovie(movie types.Movie) (uuid.UUID, error) {
	...
	if len(data.Result) == 0 {
		err = errors.New("invalid Author ID")
		zap.S().Error(err)
		return uuid.Nil, err
	}
...
func (s *service) GetMovieById(ID uuid.UUID) (types.Movie, error) {
	...
	if len(data.Result) == 0 {
		err = errors.New(fmt.Sprintf("%v not found", ID))
		zap.S().Error("no such ID", err)
		return types.Movie{}, err
	}
...
func (s *service) GetAuthors() ([]types.Author, error) {
	...
	_ = types.Author{AuthorID: uuid.UUID{}}
	body, err := s.SendRequest("GET",
...
func (s *service) GetAuthorById(AuthorID uuid.UUID) (types.Author, error) {
	...
	url := fmt.Sprintf("http://%s:2480/query/%s/sql/select from Author where ID = '%s'", s.orientDBHost, s.orientDBName, AuthorID)
	...
	if len(data.Result) == 0 {
		err = errors.New(fmt.Sprintf("%v not found", AuthorID))
		zap.S().Error("no such ID", err)
		return types.Author{}, err
	}
